					...
					<table class="table">
						...
						<c:choose>
							...
							<c:otherwise>
									...
									<c:if test="${row.status=='有效' }"><button type="button" onclick="return updateColumnstatus('无效','${row.id}');" class="btn btn-warning">设定无效</button></c:if>
									<c:if test="${row.status=='无效' }"><button type="button" onclick="return updateColumnstatus('有效','${row.id}');" class="btn btn-warning">设定有效</button></c:if>
								</td>
								<td>
